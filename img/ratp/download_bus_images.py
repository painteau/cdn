...
def download_bus_image(bus_number):
    url = f"https://fr.m.wikipedia.org/wiki/Fichier:Logo_Ligne_Bus_RATP_{bus_number}.svg"
    ...
    try:
        ...
        if img_tags:
            ...
            img_url = "https:" + img_tag['href']
            print(f"URL de l'image pour la ligne {bus_number}: {img_url}")
            ...
            file_path = f"bus/{bus_number}.png"
            with open(file_path, 'wb') as f:
                f.write(img_response.content)
            print(f"Image pour la ligne {bus_number} téléchargée avec succès.")
        else:
            print(f"Image pour la ligne {bus_number} non trouvée.")
